...
import { Gift, Plus, Trash2, DollarSign, Link as LinkIcon, ShoppingBag, ArrowLeft } from 'lucide-react';
...
export const WishlistPage: React.FC = () => {
  ...
  const isMe = user && (participant.userId === user.id || participant.email === user.email);
  const isMySecretSanta = group.participants.find(p => p.assignedToId === participantId && (p.userId === user?.id || p.email === user?.email));
  ...
  const handleAddItem = async () => {
    ...
    await updateParticipant(groupId!, participantId!, { wishlist: updatedWishlist });
    setNewItem({});
    setIsModalOpen(false);
  };

  const handleRemoveItem = async (itemId: string) => {
    const updatedWishlist = participant.wishlist.filter((item) => item.id !== itemId);
    await updateParticipant(groupId!, participantId!, { wishlist: updatedWishlist });
  };

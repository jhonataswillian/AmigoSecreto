...
export const DrawRevealPage: React.FC = () => {
  ...
  const myParticipant = currentGroup.participants.find(
    (p) => p.userId === user.id || p.email === user.email,
  );
  const myMatch = currentGroup.participants.find(
  ...
  if (!myParticipant || !myMatch) {
    return (
      <div className="p-8 text-center">
        <p>
          Você não está participando deste sorteio ou ele ainda não aconteceu.
        </p>
  ...
  return (
    <div className="min-h-screen p-4 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="text-center mb-8 relative z-10">
        ...
        <p className="text-gray-600">Prepare o presente com carinho!</p>
      </div>
          ...
          >
            <div className="w-32 h-32 rounded-full bg-christmas-green mx-auto mb-6 overflow-hidden border-4 border-christmas-gold shadow-lg">
              <img
                src={
                  myMatch.avatar ||
                  `https://api.dicebear.com/7.x/avataaars/svg?seed=${myMatch.name}`
                }
                alt={myMatch.name}
            ...
            <p className="text-gray-500 mb-6">{myMatch.email}</p>
